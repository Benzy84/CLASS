...
def select_files_or_folder():
    ...
    if messagebox_result == 'yes':
        ...
        if folder:
            ...
            return files, folder, has_ref_images, ref_folder, ref_files
        return [], None, False, None, None
    else:
        files = filedialog.askopenfilenames(filetypes=[("Image files", "*.jpg;*.tif;*.png")])
...
def create_results_directory(base_folder, has_ref_images):
    ...
    subfolders = ['cropped_images', 'reconstructed_amplitudes', 'reconstructed_phases', 'complex_matrices']
    if has_ref_images:
        subfolders.append('normalized_fields')
...
def save_results_parallel(paths, base_filename, cropped_image, reconstructed_amplitude, reconstructed_phase,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        futures = [
            ...
            executor.submit(save_file, os.path.join(paths['complex_matrices'], f"{base_filename}_matrix.npy"),
                            reconstructed_field, False)
        ]

        if normalized_field is not None:
            futures.append(executor.submit(save_file,
                                           os.path.join(paths['normalized_fields'], f"{base_filename}_normalized.npy"),
...
# Main execution
files, folder, has_ref_images, ref_folder, ref_files = select_files_or_folder()
