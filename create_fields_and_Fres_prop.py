...
def roll_fft(event):
    ...
    if ax == ax1:
        ...
        rows_to_roll = array_1_fft.shape[0]//2 - y
        cols_to_roll = array_1_fft.shape[1]//2 - x
        array_1_fft_rolled = torch.roll(array_1_fft, shifts=(rows_to_roll, cols_to_roll), dims=(0, 1))
        ax1.clear()
        phplot(array_1_fft_rolled, ax1, log_scale=True)
        ax1.set_title('Array 1 FFT')
    elif ax == ax2:
        y, x = int(event.ydata), int(event.xdata)
        rows_to_roll = array_2_fft.shape[0]//2 - y
        cols_to_roll = array_2_fft.shape[1]//2 - x
        array_2_fft_rolled = torch.roll(array_2_fft, shifts=(rows_to_roll, cols_to_roll), dims=(0, 1))
        ax2.clear()
        phplot(array_2_fft_rolled, ax2, log_scale=True)
...
ax1.imshow(torch.abs(array_1_fft_rolled).cpu().numpy(), cmap='viridis')
ax1.set_title('Array 1 FFT')
ax2.imshow(torch.abs(array_2_fft_rolled).cpu().numpy(), cmap='viridis')
ax2.set_title('Array 2 FFT')
